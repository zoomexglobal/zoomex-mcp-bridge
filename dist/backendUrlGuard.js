...
export function assertSafeBackendUrl(rawUrl) {
    ...
    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
        throw new BackendUrlGuardError(`Backend URL must use http: or https: (got ${parsed.protocol})`);
    }
    if (parsed.username || parsed.password) {
        throw new BackendUrlGuardError("Backend URL must not embed credentials (user:pass@host)");
    }
    ...
    if (!isLoopbackHost(host)) {
        throw new BackendUrlGuardError(`Insecure HTTP to non-loopback host "${parsed.hostname}" is not allowed. ` +
