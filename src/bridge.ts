...
/**
 * 持有可变连接状态的上下文，供 handler 闭包共享。
 */
interface ConnectionContext {
  client: Client;
  backendUrl: URL;
  /** 当前正在进行的重连 Promise，null 表示连接正常 */
 ...
 */
async function reconnect(ctx: ConnectionContext): Promise<void> {
 ...
 */
function ensureReconnected(ctx: ConnectionContext): Promise<void> {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (isSessionError(err) && attempt < MAX_RETRIES) {
        ...
        log(
          `WARN: ${label} failed (session error, attempt ${attempt + 1}/${MAX_RETRIES}): ` +
...
export async function startBridge(config: BridgeConfig): Promise<void> {
  ...
  try {
    ...
    const detail =
      err instanceof BackendUrlGuardError ? err.message : formatError(err);
  ...
  try {
    ...
    log(
      `ERROR: cannot connect to backend ${config.backendUrl}. ` +
  ...
  const server = new Server(
    { name: "zoomex-mcp-server", version: "0.1.0" },
    { capabilities: { tools: {} } }
  );
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    const merged = injectCredentials(
      request.params.arguments,
      credentialEnv
    );
    return withSessionRetry(
      ctx,
      () => ctx.client.callTool({ name: request.params.name, arguments: merged }),
